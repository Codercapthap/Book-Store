...
function DeliveryUpdateForm({ isOpen, setIsOpen, load, currentDelivery }) {
  const [method, setMethod] = useState(currentDelivery.method);
  const [base, setBase] = useState(currentDelivery.base);
  const [coefficient, setCoefficient] = useState(currentDelivery.coefficient);
  ...
  const submit = () => {
    if (validateForm()) {
      Location.updateDelivery(currentDelivery.id, method, base, coefficient)
        .then((res) => {
